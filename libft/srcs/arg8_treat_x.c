...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/17 21:12:11 by natakaha          #+#    #+#             */
/*   Updated: 2025/10/22 11:44:33 by natakaha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../ft_printf.h"

static char	*treat_flag_x_m(char *itoa, t_flag flag, char *dest, size_t size)
{
	size_t	icount;
	size_t	dcount;
...
static char	*treat_flag_x_z(char *itoa, t_flag flag, char *dest, size_t size)
{
	size_t	icount;
	size_t	dcount;
...
static char	*treat_flag_x_n(char *itoa, t_flag flag, char *dest, size_t size)
{
	size_t	icount;
	size_t	dcount;
	...
	{
		while (dcount + flag.precision + 2
			< size && dcount + ft_strlen(itoa) + 2 < size)
			dest[dcount++] = ' ';
		dest[dcount++] = '0';
		dest[dcount++] = 'x';
	}
...
static char	*treat_flag_x1(char *itoa, t_flag flag, char *dest, size_t size)
{
	if (flag.minus)
...
char	*treat_flag_x(char	*itoa, t_flag flag)
{
	...
	if (flag.sharp && itoa[0] != '0')
		size = return_maximum(flag.precision
				+ 2, ft_strlen(itoa) + 2, flag.width);
	else
